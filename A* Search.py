...
# start = np.array([842, 160]) # start point for newyork city
# goal = np.array([95, 518]) # central park for newyork city
start = np.array([582, 870]) #waterfront for pittsburgh
goal = np.array([332, 723]) #Carnegie Mellon University for pittsburgh
...
class AStarSearch:
    ...
    def heuristic(self, move):
        answer = (((move[0] - goal[0])**2) + ((move[1] - goal[1])**2))**0.5
    ...
    def valid_move(self, move):
        ...
        if self.grid[move[0], move[1]] < 0.4:
            return False
        return True
